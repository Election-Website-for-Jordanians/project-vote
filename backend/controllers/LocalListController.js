...
exports.createLocalListing = async (req, res) => {
  try {
    ...
    if (error.name === 'SequelizeUniqueConstraintError') {
      res.status(400).json({ error: 'A local listing with this name already exists.' });
    } else {
...
exports.getlocalListingInformation = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Error retrieving local Listing Information' });
  }
};
...
exports.getLocalListing = async (req, res) => {
  ...
  try {
    ...
    if (!localList) {
      return res.status(404).json({ message: 'No local listing found for the given ID.' });
    }
...
exports.getCitizen = async (req, res) => {
  ...
  try {
    ...
    // التأكد من أن هناك بيانات
    if (!citizen) {
      return res.status(404).json({ message: 'No citizen found for the given ID.' });
    }
